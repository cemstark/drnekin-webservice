  ...
  <main class="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <dl class="grid gap-4 px-6 py-6 sm:grid-cols-2 lg:grid-cols-3 text-sm">
        ...
        <div><dt class="text-xs font-semibold uppercase tracking-wide text-slate-500">Son hatirlatma</dt><dd class="mt-1 text-slate-950"><?= e($record['policy_reminder_sent_at'] ?: '-') ?></dd></div>
    ...
    <section class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div class="px-6 py-5">
        <form method="post" action="<?= e(panel_url('upload_attachment.php')) ?>" enctype="multipart/form-data" class="mb-5 grid gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 sm:grid-cols-[180px_1fr_auto] sm:items-center">
          ...
          <button class="h-10 rounded-lg bg-blue-600 px-4 text-sm font-bold text-white hover:bg-blue-700" type="submit">Yukle</button>
        </form>

        <div class="mb-4 flex flex-wrap gap-2">
          <a href="<?= e(panel_url('view.php?id=' . $id)) ?>" class="<?= $categoryFilter === '' ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50' ?> inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold">Tumu</a>
          <?php foreach ($categories as $key => $label): ?>
            <a href="<?= e(panel_url('view.php?id=' . $id . '&cat=' . $key)) ?>" class="<?= $categoryFilter === $key ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50' ?> inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold"><?= e($label) ?></a>
          <?php endforeach; ?>
        </div>
            ...
            ?>
              <div class="overflow-hidden rounded-xl border border-slate-200 bg-white">
                ...
                <div class="p-3">
                  ...
                  <p class="text-xs text-slate-500"><?= e($att['uploaded_at']) ?><?= $att['uploaded_by_name'] ? ' &mdash; ' . e($att['uploaded_by_name']) : '' ?></p>
                  <div class="mt-3 flex items-center justify-between">
                    ...
                    <form method="post" action="<?= e(panel_url('delete_attachment.php')) ?>" onsubmit="return confirm('Dosyayi sil?');">
                      ...
                      <button class="text-sm font-semibold text-red-600 hover:underline" type="submit">Sil</button>
                    </form>
                  </div>
                </div>
              </div>
